...
{//创建这个，然后其他模块都继承他，这是为了在通过参数设置使用哪个模块时，可以通用的处理
...
/* simple BLOBLIST
团块链表：只是cvseq 对Blob的包装，增加了GetBlobByID，返回 对应ID的团块。
*/
...
/* simple TRACKLIST */
/*
团块：struct CvBlob ，描述一个团块。
团块链表类：class  CvBlobSeq   是对 结构体CvBlob使用seq包装的。 描述团块组成的链表
跟踪链结构体： struct CvBlobTrack 是对CvBlobSeq扩展。   描述一个目标在各帧的blob位置。
跟踪链表类：class  CvBlobTrackSeq 。
有一个CvSeq链表： 用来装CvBlobTrack。 每个元素都是一个目标。
是对多个目标的描述和操作。
*/
...
CV_EXPORTS CvBlobDetector* cvCreateBlobDetectorSimpleOfMy();
CV_EXPORTS CvBlobDetector* cvCreateBlobDetectorCC();
//这两个函数对应enteringblobdetection。Cpp的具体实现，两个类都有单独函数来创建。
...
public:
	...
	/*向跟踪器里添加一个目标，目标有pBlob给出， 完整图， 和前景作为输入	*/
...
///////////////////////////////////////////////////////////////////////////////
//自己的实现
CV_EXPORTS CvBlobTracker* cvCreateBlobTrackerListColorTracker();
...
CV_EXPORTS CvBlobTrackerAuto* cvCreateBlobTrackerAuto1();
CV_EXPORTS CvBlobTrackerAuto* cvCreateBlobTrackerAuto1(CvBlobTrackerAutoParam1* param);
// = NULL 如果搞上 这个，那么调用 cvCreateBlobTrackerAuto1()就可能是没参数，也可能是使用默认的param=NULL。编译器无法判断了。
//这看来，c++重载上，出现了默认构造函数，就不能再出现一个有默认值的构造函数了。！！！
...
public:
	...
	void showAndSaveResult(IplImage* pImg);
	void ProcessTracker(IplImage* pImg, IplImage* pMask = NULL);
	void ProcessNewBlob(IplImage* pImg, IplImage* pMask = NULL);

	/* 无法识别mat
	void showAndSaveResult(Mat  pImg);//MAT处理
	void ProcessTracker(Mat pImg, Mat pMask );
	void ProcessNewBlob(Mat pImg, Mat pMask );
*/
	void Process(IplImage* pImg, IplImage* pMask = NULL);
	void Process__(IplImage* pImg, IplImage* pMask = NULL);
	void Release(){
...
private:
	...
	CvBlobTrackPostProc*    m_pBTPostProc;//前处理
	int                     m_UsePPData;
	CvBlobTrackAnalysis*    m_pBTA; /* Blob trajectory analyser. */


	CvBlobSeq               m_BlobList;//团块链表
	int                     m_FrameCount;
	int                     m_NextBlobID;
