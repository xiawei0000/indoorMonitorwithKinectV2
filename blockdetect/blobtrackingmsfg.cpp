...
public:
	CvMat*          m_pHist;
	DefHistType     m_HistVolume;//float类型的，代表什么？？？
...
private:
	...
	*/
...
#endif
		m_Dim = Dim;//3个维度
		m_BinNum = (1 << BinBit);//这里是1左移5=32 不是5左移一位=两倍。。。
		m_BinNumTotal = cvRound(pow((double)m_BinNum, (double)m_Dim));
		//每个维度5位表示，3个维度，共32768个bins
		/*
	...
	{// 通过pBlob区域的像素值，映射到彩色图的直方图bin， 计算坐标处的权值k*是否为前景。 对直方图bins +K;
		//结果保存到最后的参数pHist
		int UsePrecalculatedKernel = 0;
		int BW = cvRound(pBlob->w);
		int BH = cvRound(pBlob->h);
		DefHistType Volume = 0;
		int         x0 = cvRound(pBlob->x - BW*0.5);//x是团块中心，这里得到的是左上角点
		...
		if ((x0 + BW) >= pImg->width) BW = pImg->width - x0 - 1;//防止溢出
		...
		if (m_Dim == 3)
		{
			for (y = 0; y<BH; ++y)
			{
				...
				{
					DefHistType K;
					int index = HIST_INDEX(pImgData);//获得了三通道像素值 映射到直方图的index
					...
					{
						float dx = (x + x0 - pBlob->x) / (pBlob->w*0.5f);//坐标- 中心 /宽度一半
						float dy = (y + y0 - pBlob->y) / (pBlob->h*0.5f);
						//得到的是一个比例距离， 坐标离中心距离 比上 宽高的一半。
						double r2 = dx*dx + dy*dy;
						K = (float)GetKernelHist(r2);//距离比》1的=0 ； 《1 的 = 1-r2
						//那么越近的K越大。
					}

					if (pMaskData)//前景不空，
					{//获取前景掩码 对应坐标的值， 同时乘权值。 那么是前景的才k！=0，其他都是0
						K *= pMaskData[x] * 0.003921568627450980392156862745098f;
					}
					Volume += K;
					((DefHistType*)(pHist->m_pHist->data.ptr))[index] += K;
					//得到彩色图的直方图位置， 计算坐标处的权值k。 对直方图bins +K;
				}   /* Next column. */
			}   /*  Next row. */
		}   /* if m_Dim == 3. */

		pHist->m_HistVolume = Volume;//是所有k的累积， k是距离比例值* 前景* 比例的一个值

	};  /* CollectHist */

	double calcBhattacharyya(DefHist* pHM = NULL, DefHist* pHC = NULL, DefHist* pHT = NULL)
	{//计算前两个直方图的bhatta距离 ， 后一个无效
...
protected:
	...
	{//计算pBlob周围的直方图，然后用它和 目标模型计算距离。
		if (pHist == NULL)pHist = &m_HistTemp;
		CollectHist(pImg, pImgFG, pBlob, pHist);//收集blob周围 直方图，保存到pHist。 
		return calcBhattacharyya(&m_HistModel, pHist, pHist);//计算hist模型和新计算的直方图 的bhatta距离 ， 后一个无效
	}
...
public:
	...
	{
		//计算当前pBlobPrev 区域的直方图
		//循环处理
		//统计目标的每一个点 和 这一帧新团块的每一个点 的偏差，作为坐标的偏差累积。得到新团块可能的最佳位置
		//如果得到的新位置 和目标的距离变小，将new目标位置 不断向 原目标移动，

		//如果没有碰撞，那么就可以按照团块的meanshift计算中心和尺寸了
		...
		for (iter = 0; iter<m_IterNum; ++iter)
		{
			...
			CollectHist(pImg, NULL, &m_Blob, &m_HistCandidate);//计算直方图保存到m_HistCandidate 里
			B0 = calcBhattacharyya();
			//无参数时：计算的是 &m_HistModel和m_HistCandidate 的距离
			...
			if (m_Dim == 3)
			{
				...
				for (y = 0; y<m_ObjSize.height; ++y)
				{
					...
					for (x = 0; x<m_ObjSize.width; ++x, pImgData += 3)
					{
						...
						double      HM = 0;
						double      HC = 0;
						int index = HIST_INDEX(pImgData);//图片像素在直方图的索引
						assert(index >= 0 && index < m_BinNumTotal);
						...
						if (HC>0)//防止除零
						{
							double V = sqrt(HM / HC);//v就说明了 目标和这一帧 对应坐标处 的偏差
							// 就是上一帧和这一帧区域，对应坐标 像素值的比：只是先映射到直方图，求权值比例
							//这样可以抗光照？？
							if (!m_Collision && m_FGWeight>0 && pMaskData)
							{
								V += m_FGWeight*(pMaskData[x] / 255.0f);
							}
							K *= (float)MIN(V, 100000.);//由中心到外界，每个偏差都有权值的。
						}

						sum += K;
						newx += K*x;//这一帧的坐标，是每一个点坐标x * 这个坐标和原目标偏差 权值K的累积
						newy += K*y;
					}   /* Next column. */
				}   /*  Next row. */
			...
			{   /* Iterate using bahattcharrya coefficient: */
				...
				B1 = calcBhattacharyya();//计算new位置和目标的距离，如果小就更新
				if (B1 > B0) break;
				newx = 0.5f*(newx + m_Blob.x);//原目标和新目标个一半权值
	...
	{// 得到的是目标直方图和团块直方图的匹配度
		double  S = 0.2;
		double  B = GetBhattacharyya(pImg, pImgUnusedReg, pBlob, &m_HistTemp);//得到blob直方图和目标模型的距离
		return exp((B - 1) / (2 * S));

	};  /*CvBlobTrackerOneMSFG::*/

	virtual void Update(CvBlob* pBlob, IplImage* pImg, IplImage* pImgFG = NULL)
	{   /* Update histogram: *///用团块区域更新目标的直方图。 一个目标对应一个msfg类
...
{
public:
	CvBlobTrackerOneMS()//ONEMS类是没有前景处理模块
	{
		SetParam("FGWeight", 0);
		DelParam("FGWeight");
		SetModuleName("MS");
...
CvBlobTracker* cvCreateBlobTrackerMS()//只用均值漂移，不用前景
{
...
private:
	...
	CvSize          m_ImgSize;
	CvBlob          m_Blob;//团块
	DefParticle*    m_pParticlesPredicted;//预测的粒子
	DefParticle*    m_pParticlesResampled;//重采样的粒子
	CvRNG           m_RNG;//uint64的随机数
#ifdef _OPENMP
	int             m_ThreadNum;
...
private:
	...
	{/* 是一个随机产生粒子的函数。
	 //取随机数填
	 //1：大约一半的粒子是直接用目标团块的位置
	 //2：随机的让粒子按照速度方向移动，或速度=0
	 //3：按照尺寸 ，随机的移动粒子的xy坐标。 改变粒子的xy方向速度
	 //4：随机修改尺寸
	 //5：防止太小，太大，溢出图
	 */
		int p;
		...
		{   /* "Prediction" of particle: */
			...
			//用随机数填充数组rm 并更新 RNG 状态  后两个参数是上下界
			m_pParticlesPredicted[p] = m_pParticlesResampled[p];

			if (cvRandReal(&m_RNG)<0.5)//cvRandReal 返回浮点型 0..1 随机数并更新 RNG
			{   /* Half of particles will predict based on external blob: */
				m_pParticlesPredicted[p].blob = m_Blob;
			}//大约一半的粒子是直接用目标团块的

			//随机的让粒子按照速度方向移动，或速度=0
			if (cvRandReal(&m_RNG)<m_UseVel)//粒子按照速度移动坐标
			...
			{   /* Stop several particles: *///粒子的速度为0
				m_pParticlesPredicted[p].Vx = 0;
				m_pParticlesPredicted[p].Vy = 0;
			}

			{   /* Update position: */
				//按照尺寸 ，随机的移动粒子的xy坐标。 改变粒子的xy方向速度 
			...
			//防止太小，太大，溢出图
			/* Truncate 截短 缩短 size of particle: */
...
#pragma omp parallel for num_threads(m_ThreadNum) schedule(runtime)
		...
		{   /* Calculate weights for particles: */
			...
			m_pParticlesPredicted[p].W *= exp((B - 1) / (2 * S));//粒子的权值就是 直方图距离的函数了
...
#pragma omp parallel for
	...
	void Resample()
	{   /* Resample particle: */
		//重采样 根据粒子的权值，随机采样，赋值到m_pParticlesResampled， 权值=1了。
		int         p;
		double      Sum = 0;
