...
#include "precomp.hpp"
/*这个的方法大概是 用背景的团块组一个链表， 原有的跟踪目标组成一个链表。 
如果有交集的
然后 匹配 用直方图来匹配，获得最佳位置。。*/
...
typedef struct DefBlobTrackerCR
{
    ...
    int                         Collision;
    CvBlobSeq*                  pBlobHyp;
    CvBlobTrackerOne*           pResolver;//跟踪器 ，算法中设置为粒子滤波
} DefBlobTrackerCR;
...
public:
    ...
    {
        m_CreateCR = CreateCR;//这里就设置好了cvCreateBlobTrackerOneMSPF。 使用meanshift来跟踪。
	...
	{	//第一步：从前景里提取团块：位置+大小 ==》保存到m_BlobListNew链表里
		...
		//第三步：标记每个目标 可能的下一个位置 添加到pBlobHyp
		//用新的链每个块和 已跟踪的目标预测位置 比较，如果有交集 就添加到目标的可能链表pBlobHyp里 
		//和cc不同的地方：
		//第四步： 用已经跟踪的目标链表 元素 和他可能的位置一个个直方图匹配。
		//然后更新目标的直返图
        CvSeq*      cnts;
        CvSeq*      cnt;
        //CvMat*      pMC = NULL;
...
//                            double  dy = fabs(CV_BLOB_Y(pB)-CV_BLOB_Y(pBNew));
            ...
            }   /* Non-collision tracking. *///初始化粒子， blob团块都取值传入的blob

            pBT->pResolver->Update(pB, pImg, pImgFG);
			//检测器的更新：对pb团块区域的前景像素==》映射到直方图bin++
