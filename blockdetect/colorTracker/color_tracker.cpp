...
vector<vector<double> > ColorTracker::w2c(0);
vector<vector<double> > ColorTracker::w2c_t(0);
...
{//载入文件
	...
	if (w2c.size() == 0)
	{
		...
		for (int i = 0; i < 10; i++)//10行 3w列
		{
			w2c_t.push_back(vector<double>(32768, 0));
		}
		vector<double> tmp(10, 0);//3w行，10列
...
void ColorTracker::setParam(ColorTrackerParameters &param)
{
	params = param;
}
...
{//计算了一个投影矩阵
	...
	for (int i = 0; i < sz; i++)
	{
		Mat tmp = z[i].mul(cos_window);//按位点乘 z的每个矩阵 * coswindow
		z[i] = tmp;
	}
	return z;
}
...
{//indeces 是 inp 数组的索引，返回取到的值数组
	int sz = min(inp.size(), indeces.size());
...
{//没懂
	...
	vector<Mat> im_split;//c++的分离通道，很简单
...
void ColorTracker::init_tracking()
{
	...
	use_dimensionality_reduction = (params.compressed_features.size() != 0);//设置了压缩

	target_sz = cv::Size(params.wsize.width, params.wsize.height);
	pos = cv::Point(params.init_pos.x, params.init_pos.y);

	// window size, taking padding into account  扩大的窗口
	...
	if (sz_with_padding.width >target_sz.width)
	{
		sz_with_padding.width = target_sz.width;
	}
	if (sz_with_padding.height>target_sz.height)
	{
		sz_with_padding.height = target_sz.height;
	}
	...
	//y = exp(-0.5 / output_sigma^2 * (rs.^2 + cs.^2));%每个坐标的系数值
	...
	//两个矩阵直接乘，得到hann窗口， opencv的mat也支持 矩阵乘法的。
